...
## Operational note
In `C:\\Users\\Administrator\\Documents\\My Work\\Strategies\\EMAwave34`, stage all changes, create a commit with an appropriate message, then force‑push the local master branch so it overwrites both origin/main and origin/master, ensuring the remote repo kirkvan/EMAwave34 exactly matches the local folder.

## Latest changes (2026-02-12)
- **Repo sync prep:** README updated with recent work summary; local `master` will be force‑pushed to overwrite `origin/main` and `origin/master` so the remote matches the local folder.
